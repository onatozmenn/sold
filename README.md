...
> **Tenth real live run, `FAIL`; the viewer barrier is crossed and Fix 10 makes field extraction label-bounded and mojibake-safe.** On the tenth run the collector reached a **stable DOM-text representation** for both real viewers: `auction_result` transitioned `image_only → dom_text → dom_text` and `sale_notice` reached `dom_text`, both `viewer_ready_state=stable_text_representation` with `viewer_asset_captured=false` and `document_source_artifact_collected=true`, so `artifact_types_collected` now holds `auction_result`, `sale_notice`, and `status_card` (the byte-identical 298×298 PNG was transitional, not the source). Same-asset **reconciliation succeeded** (`ada=50984`, `parsel=1`, `section_no=60`, `floor=12`) and **KDV `20.0` was deterministically extracted** — proving the collected source is real, structured official text. But extraction still failed at the field level: appraisal was **ambiguous** (`[1.0, 6800000.0]`), the explicit **İhale Bedeli was missing**, **ALACAĞA MAHSUBEN** was not recognized, and the audit stayed `PENDING_REVIEW`. Two structural, code-evident causes: (1) appraisal/İhale extraction took the **first number in a wide window** with no monetary-format or field-boundary constraint, so a bare `1` (a parcel/row identifier) was admitted as an appraisal candidate; and (2) `extract.py` applied **no mojibake repair** (unlike `collect.py`), so Turkish-special-character labels (`İhale Bedeli`, `ALACAĞA`) silently failed to match while pure-ASCII ones (`Muhammen Bedel`, `KDV`, `ada`, `parsel`) matched — exactly the observed pass/fail split. **Fix 10** (stabilized-source finalization + label-bounded field extraction only; no OCR, no ML, no known-truth fallback, and no structural change): (a) the ordinary per-attempt viewer diagnostics now describe the **final stabilized** state (`final_viewer_representation=dom_text`, `final_viewer_text_available=true`, `final_viewer_outcome=content_available`) with the pre-stabilization snapshot retained separately (`initial_viewer_representation=image_only`, `initial_viewer_outcome=image_backed`) and the `image_only → dom_text` transition sequence preserved; (b) official monetary fields are extracted **only** from a recognized field label bound to an immediate value region (same segment, or the adjacent segment for a `LABEL`/`VALUE` block split), and a value must be a **Turkish monetary literal** (grouping dots and/or a decimal comma) — a bare integer such as a parcel, row, section, or `ada` number is **structurally excluded** (not by any amount threshold, `max()`, or verifier value), which removes the spurious `1.0`; (c) the explicit **İhale Bedeli** is recovered from its own label/value relation (never `Satış Tutarı`, `Ödenmesi Gereken Bedel`, or an `ALACAĞA MAHSUBEN` phrase); (d) **ALACAĞA MAHSUBEN** is recognized from the actual settlement-field text (including block-split and constrained mojibake-repaired forms), never inferred from price/creditor/zero-balance; and (e) `extract.py` now applies the repository's constrained mojibake repair before folding, so Turkish-special-character labels match. New privacy-safe field-level diagnostics (`auction_price_field_label_found`, `auction_price_candidate_count`, `auction_price_value_relation_strategy`, `appraisal_field_label_found`, `appraisal_candidate_count`, `appraisal_value_relation_strategies`, `settlement_field_label_found`, `alacaga_mahsuben_detected`, `settlement_value_relation_strategy`) carry no full source text. Reconciliation and KDV extraction are preserved. Added 39 offline tests. Full suite: **557 passed** (518 baseline + 39 tests). **No live `PASS` is claimed: the parser has not been proven live after Fix 10 — the İhale Bedeli and appraisal have not been extracted from the real viewer, no admission has occurred, and a post-Fix-10 operator rerun is required.** Structural core, four SMM moments, `conditional_on_trade`, `Θ_A`, TOKİ external status (5 observed / 0 SMM), and the numerical-search convention are unchanged; the pilot remains non-mutating (genuine count stays 7).

> **Eleventh real live run, `FAIL`; Fix 10's appraisal fix is now live-proven, and Fix 11 persists the real stabilized source and adds a bounded field-layout adapter.** On the eleventh run Fix 10 worked live: `final_viewer_representation=dom_text`, `final_viewer_text_available=true`, `document_source_artifact_collected=true` for both `auction_result` and `sale_notice`, and the appraisal extracted cleanly — `extracted_appraisal=6800000.0`, `appraisal_candidate_count=1`, `appraisal_candidates=[6800000.0]`, `appraisal_value_relation_strategies=[same_segment]` — so the earlier spurious `1.0` is **live-resolved**. Same-asset reconciliation stayed `reconciled` (`ada=50984`, `floor=12`, `parsel=1`, `section_no=60`) and KDV stayed `20.0`. The remaining required blocker is precise: `auction_price_field_label_found=true` but `auction_price_candidate_count=0` and `extracted_auction_price=null` — the real stabilized source **contains a recognized İhale Bedeli label, but the current label→value relation does not reach a money value** — and on the settlement side `settlement_field_label_found=false` / `alacaga_mahsuben_detected=false`, so the audit stayed `PENDING_REVIEW`. Because the previous task never persisted the raw stabilized DOM text, the exact real serialization could not be inspected. **Fix 11 (source persistence + a bounded field-layout adapter only; no OCR, no ML, no known-truth injection, and no page-state/card/modal/view-action/viewer/reconciliation/audit/structural change):** (a) when a viewer reaches `stable_text_representation` and a document-specific DOM-text source is collected, the **exact source text is now persisted locally** through the existing gitignored ingestion store (`artifacts/viewer_sources/`), and only privacy-safe provenance (`source_text_persisted`, `source_text_artifact_sha256`, `source_text_artifact_size`) is recorded — the body is **never** committed, copied into README/DEVELOPMENT_HISTORY, emitted into pilot JSON, printed in full, or placed in test fixtures; (b) the İhale Bedeli extractor now supports a **bounded multi-segment label→value relation** (`same_segment → adjacent_segment → bounded_following`, inspecting at most a few following segments and **stopping immediately** at the next recognized field label or property identifier), still accepting only a Turkish monetary literal, with **no** whole-document scan, `max()`, threshold, or verifier value; (c) settlement recognition adds a **bounded token-sequence matcher** requiring the complete `odenmesi → gereken → bedel` identity within a small segment window (a generic `bedel`/`gereken` never qualifies), then reads `ALACAĞA MAHSUBEN` only from that field's bounded value region (including split serializations); and (d) new **privacy-safe field-neighborhood diagnostics** (`field_neighborhood` with only structural counts, boundary-stop reasons, and normalized field-label *types* — never segment text, money, or personal data) let the next live report explain the actual İhale Bedeli and settlement layout without exposing content. Fix 10's live-proven appraisal (`6800000.0`, one candidate) and viewer finalization, plus reconciliation and KDV, are preserved unchanged; the explicit İhale Bedeli requirement is not weakened (`Satış Tutarı`/`Ödenmesi Gereken Bedel`/status-card amounts are never the auction price). Added 33 offline tests. Full suite: **590 passed** (557 baseline + 33 tests). **No live `PASS` is claimed: the İhale Bedeli value has not been extracted from the real viewer, the settlement field is not yet confirmed from the real source, no admission has occurred, and a post-Fix-11 operator rerun (which will persist and reveal the real layout) is required.** Structural core, four SMM moments, `conditional_on_trade`, `Θ_A`, TOKİ external status (5 observed / 0 SMM), and the numerical-search convention are unchanged; the pilot remains non-mutating (genuine count stays 7).

> **Twelfth real live run, `FAIL`; a manually downloaded real official `.udf` reveals a deterministic native source path, and Fix 12 adds native UDF container extraction.** Run 12's viewer sequence for `auction_result` ended `image_only → unknown → unknown` within the observation window, so no auction-result DOM text was collected. The operator then **manually clicked the real row-local download** for `1- Artırma Sonuç / Uzatma Tutanağı` and supplied the exact `.udf`, which was inspected byte-for-byte: **4406 bytes, a ZIP-compatible (`PK`) container** whose top-level members are `documentproperties.xml`, `content.xml`, and `sign.sgn`; the **`content.xml` (17940 bytes uncompressed) is UTF-8 XML carrying the official document source text directly inside its `content` element's CDATA** — including the explicit `İhale Bedeli` monetary literal, `Ödenmesi Gereken Bedel` / `ALACAĞA MAHSUBEN`, KDV, and the same-asset descriptors. Root cause: for the measured auction-result document the **viewer is not the best deterministic evidence layer** — the official downloaded `.udf` itself is a native container with deterministic text. **Fix 12 (deterministic native UDF container extraction + same-row official download collection only; no OCR, no ML, no rendering/LibreOffice/GUI, no known-truth injection, and no structural change; the viewer path and its architecture are preserved, not reopened):** a new [udf.py](src/sold/ingestion/uyap/udf.py) `extract_udf_source_text(path_or_bytes)` validates the ZIP container, reads **only** the root `content.xml` directly (no `extractall`, rejecting path-traversal members, duplicate/ambiguous `content.xml`, encrypted/malformed archives, and bounding the decompressed size against zip-bombs), parses the XML **safely** (no external entities/DTD/XInclude/network — a `DOCTYPE`/`ENTITY` is refused outright), and returns the `content` element's CDATA/text exactly, with honest per-stage blockers. The collector now, for a recognized `auction_result` DocumentRow whose **row-local download action is positively resolved by the existing Fix-6 semantics** and which emits a **real browser download event**, prefers the native download path (`_collect_native_udf_download`) — this is a **positively-resolved same-row official download** (never a page-global/first/Nth control, and *not* an "unsupported viewer ⇒ download" fallback), storing the exact `.udf` bytes unchanged in the existing gitignored artifact store and feeding the extracted `content.xml` text into the **existing** label-bounded `extract_evidence`. Native-UDF support is honest — it requires successful container validation and `content.xml` extraction, never the `.udf` extension alone. The viewer path remains available as the secondary/compatibility route, `sale_notice`'s working stable-DOM-text path is unchanged, and the audit numerator stays the explicit official `İhale Bedeli` only (`Satış Tutarı`/`Ödenmesi Gereken Bedel`/`ALACAĞA MAHSUBEN` are never the auction price). New privacy-safe per-attempt diagnostics (`native_download_*`, `native_artifact_*`, `native_udf_*` — container kind, ZIP validity, a **known-member-name summary**, `content.xml` presence/size, XML-parse/content-element flags, source-text availability, short SHA256, and blocking reasons) carry **no** native source text, full `content.xml`, names, IDs, addresses, URLs, cookies, or tokens. Added 37 offline tests using **sanitized synthetic** UDF fixtures (synthetic amounts/identifiers — the real official document and its `content.xml` are never committed). Full suite: **627 passed** (590 baseline + 37 tests). **The manually supplied artifact proves the measured native format only; it does *not* yet prove the automated row-local browser download + native extraction path works live, and no pilot `PASS` is claimed — a post-Fix-12 operator rerun is required.** Structural core, four SMM moments, `conditional_on_trade`, `Θ_A`, TOKİ external status (5 observed / 0 SMM), and the numerical-search convention are unchanged; the pilot remains non-mutating (genuine count stays 7).

> **Thirteenth real live run, `FAIL`; the native UDF download path works but downloaded the wrong row's document — Fix 13 binds the download to the DocumentRow and corroborates the native document type.** Run 13 live-proved the native path technically: `native_download_event_detected=true`, `native_artifact_collected=true`, `native_artifact_extension=.udf`, `native_udf_zip_valid=true`, `native_udf_content_xml_found=true`, `native_udf_xml_parse_succeeded=true`, `native_udf_content_element_found=true`, `native_udf_source_text_available=true`. But direct byte/container inspection of the exact automated artifact (`12109` bytes) proved it is the **SALE NOTICE** UDF — its `content.xml` title is `TAŞINMAZIN ELEKTRONİK SATIŞ ORTAMINDA AÇIK ARTIRMA İLANI` with `Kıymeti` + `KDV`, **not** the manually downloaded correct `Artırma Sonuç / Uzatma Tutanağı` result document (`4406` bytes, with explicit `İhale Bedeli` and `Ödenmesi Gereken Bedel : ALACAĞA MAHSUBEN`). The automated `auction_result` path **downloaded the sale-notice UDF and misbound it as `auction_result`**, and the sale-notice boilerplate's inflected prose (`ihale bedelini`/`ihale bedelinin`) falsely set `auction_price_field_label_found=true` with `auction_price_candidate_count=0`. Root cause: **DocumentRow identity was not preserved through the native download click** — `_locate_row_download_action` filtered rows by a broad `has_text` label and returned the *first* matching download control, which (in DOM order, sale-notice first) belonged to a different row. **Fix 13 (DocumentRow-bound native download + native document-type corroboration only; native UDF parsing, viewer stabilization, and appraisal parsing are unchanged; no OCR, no ML, no known-truth injection):** (a) before downloading, the collector reacquires the **unique** logical row by recognized identity (`select_unique_document_row` — matching `artifact_type` + normalized label with `logical_row_recognized_type_count == 1`; an ambiguous or multi-identity row means **no download**, with an honest blocker); (b) `_locate_row_download_action` now requires the row-scoped download control to be **exactly one** (a match count > 1 means the "row" is really a container, so it is skipped rather than grabbing the first — never a page-global/first/Nth control); (c) after the native `content.xml` text is extracted, a deterministic **document-type corroboration** (`classify_udf_document_type` → `auction_result`/`sale_notice`/`sale_spec`/`appraisal_report`/`unknown` from semantic title/field tokens such as `Artırma Sonuç/Uzatma Tutanağı` vs `Elektronik Satış Ortamında Açık Artırma İlanı` — **never** from monetary values) must match the requested `artifact_type`; on a mismatch (the measured `requested=auction_result` / `detected=sale_notice`) the artifact is **not promoted** — `document_source_artifact_collected` stays false, `auction_result` is **not** added to `artifact_types_collected`, and the exact bytes are preserved only for local diagnostics; and (d) the `İhale Bedeli` field-label recognition is tightened to a **whole-word** match so inflected prose (`ihale bedelini`/`bedelinin`) no longer sets `auction_price_field_label_found`, while an explicit `İhale Bedeli : <money>` field still yields exactly one candidate. New privacy-safe diagnostics (`native_requested_*`, `native_row_reacquired_*`, `native_action_owner_*`, `native_detected_document_type`, `native_document_type_corroborated`/`_mismatch`/`_corroboration_reason`) carry no source text, personal data, URLs, or opaque IDs. Added 30 offline tests. Full suite: **657 passed** (627 baseline + 30 tests). **Run 13 proved the native UDF download/container/text path works live but downloaded the wrong document; Fix 13 rejects that misbinding and is offline-tested only — no live correction and no pilot `PASS` is claimed until the operator reruns.** Structural core, four SMM moments, `conditional_on_trade`, `Θ_A`, TOKİ external status (5 observed / 0 SMM), and the numerical-search convention are unchanged; the pilot remains non-mutating (genuine count stays 7).

> **Fourteenth real live run, `FAIL`; Fix 13's auction-result native path is now live-proven end-to-end, and Fix 14 extends the same native path to `sale_notice`.** On the fourteenth run the `auction_result` native path worked completely live: `native_row_reacquired=true`, `native_action_owner_same_row=true`, `native_document_type_corroborated=true` (`native_detected_document_type=auction_result`), the exact correct `4406`-byte result UDF was downloaded (`native_udf_content_xml_size=17940`), and the existing field extractor independently recovered `extracted_auction_price=5715000.0` (`auction_price_candidate_count=1`, `same_segment`) with `settlement_field_label_found=true` and `alacaga_mahsuben_detected=true`. That auction-result path is solved live and is **not modified**. Run 14 failed **only** because `sale_notice` was still collected through the viewer path, whose real sequence was `image_only → unknown → dom_text` ending in `viewer_ready_state=timeout_unstable` (`viewer_representation_did_not_stabilize`), so `document_source_artifact_collected=false` → `extracted_appraisal=null`, `kdv_rate=null`, `reconciliation_status=ambiguous`, `audit_decision=MISSING_APPRAISAL` — even though the same `sale_notice` row already exposes a positively resolved row-local download action. **Fix 14 (a one-policy extension; the native UDF parser, the auction-result path, viewer stabilization, and the appraisal parser are all unchanged; no OCR, no ML, no known-truth injection):** `NATIVE_DOWNLOAD_TYPES` becomes `(auction_result, sale_notice)`, so `sale_notice` now uses the **same** Fix-13 DocumentRow-bound, pre-click-revalidated, document-type-corroborated native UDF download path — unique-row reacquisition by recognized identity, an exactly-one row-scoped download control (never a global/first/Nth control), a real `expect_download` event, the unchanged native reader, and a corroboration guard that promotes the artifact **only** when `detected == sale_notice` (a `requested=sale_notice`/`detected=auction_result` mismatch is preserved for diagnostics but not promoted). `_DOC_PRIORITY` keeps `auction_result` first and `sale_notice` second, and the viewer path remains as fallback (no viewer stabilization wait is needed once the native download succeeds). The corroborated `sale_notice` source text feeds the **existing** `extract_evidence`, which already live-proved `extracted_appraisal=6800000.0` (one candidate) and `kdv_rate=20.0` when the source is available. Added 21 offline tests. Full suite: **678 passed** (657 baseline + 21 tests). **No pilot `PASS` is claimed: the automated `sale_notice` native download + corroboration has not yet run live, so a post-Fix-14 operator rerun is required.** Structural core, four SMM moments, `conditional_on_trade`, `Θ_A`, TOKİ external status (5 observed / 0 SMM), and the numerical-search convention are unchanged; the pilot remains non-mutating (genuine count stays 7).
